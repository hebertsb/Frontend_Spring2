...
import { obtenerMisReservas, obtenerDetalleReserva, ReservaCliente } from "@/api/cliente-panel";
...
export default function ClientReservations() {
  ...
  const [reservaToCancel, setReservaToCancel] = useState<ReservaCliente | null>(null);
  ...
  const [reservaToValorate, setReservaToValorate] = useState<ReservaCliente | null>(null);
  ...
  const cargarReservas = async () => {
    ...
    try {
      setLoading(true);
      const reservas: ReservaCliente[] = await obtenerMisReservas();
      setReservations(reservas || []);
      
      if (reservas && reservas.length > 0) {
        toast({
          title: "✅ Reservas cargadas",
          description: `Se encontraron ${reservas.length} reservas`,
        });
      } else {
        toast({
          title: "Información",
      ...
      let errorMessage = "No se pudieron cargar las reservas. Intenta más tarde.";
      if (error.code === 'ERR_NETWORK' || error.message.includes('Network Error')) {
        errorMessage = "Sin conexión al servidor. Verifica que el backend esté activo.";
      } else if (error.response?.status === 500) {
        errorMessage = "Error interno del servidor. El backend puede estar experimentando problemas.";
  ...
  // Filtrar solo reservas históricas (no activas ni pendientes)
  const estadosActivos = ['PAGADA', 'EN_CURSO', 'PROXIMO', 'PENDIENTE'];
  const reservasHistoricas = reservations.filter(reserva => !estadosActivos.includes(reserva.estado));

  // Filtro visual por estado
  const reservasFiltradas = reservasHistoricas.filter(reserva => {
  ...
  const estadisticasCliente = useMemo(() => {
    ...
    const reservasPagadas = reservations.filter(r => ['PAGADA', 'CONFIRMADA'].includes(r.estado.toUpperCase())).length;
    const reservasPendientes = reservations.filter(r => r.estado.toUpperCase() === 'PENDIENTE').length;
    const reservasCanceladas = reservations.filter(r => r.estado.toUpperCase() === 'CANCELADA').length;
    const reservasCompletadas = reservations.filter(r => r.estado.toUpperCase() === 'COMPLETADA').length;
    
    const gastoTotal = reservations
      .filter(r => ['PAGADA', 'CONFIRMADA'].includes(r.estado.toUpperCase()))
      .reduce((total, reserva) => total + parseFloat(reserva.total.toString()), 0);

    const totalPersonas = reservations.reduce((total, reserva) => {
  ...
  const exportarReservas = () => {
    ...
    contenido += `• Gasto total: $${estadisticasCliente.gastoTotal.toFixed(2)}\n`;
    ...
    reservasFiltradas.forEach((reserva, index) => {
      ...
      contenido += `   Total: ${parseFloat(reserva.total.toString()).toFixed(2)} ${reserva.moneda}\n`;
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">Historial completo de transacciones: reservas, pagos, cancelaciones y reprogramaciones</p>
          </div>
          <div className="flex flex-col md:flex-row gap-2 mt-4 md:mt-0 items-center">
            ...
            <Button onClick={() => cargarReservas()} variant="outline" size="sm">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{estadisticasCliente.totalReservas}</p>
              </div>
            </div>
          </div>
          <div className="bg-green-50 p-4 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{estadisticasCliente.reservasPagadas}</p>
              </div>
            </div>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">${estadisticasCliente.gastoTotal.toFixed(2)}</p>
              </div>
            </div>
          </div>
          <div className="bg-orange-50 p-4 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{estadisticasCliente.totalPersonas}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {reservasFiltradas.length === 0 ? (
            ...
            {!loading && !busqueda && filtro === "todas" && (
              <div className="space-y-2">
                <p className="text-sm text-gray-500">
                  ¡Explora nuestros paquetes turísticos y crea tu primera reserva!
                </p>
                <a 
                  href="/paquetes" 
          ...
          reservasFiltradas.map((reserva) => (
            <div
              key={reserva.id}
              className={`bg-white rounded-lg shadow-md p-6 border-l-4 ${getEstadoColor(reserva.estado)}`}
            >
              <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-2xl">{obtenerIconoTipoTransaccion(reserva.tipo_transaccion)}</span>
                    <h3 className="text-lg font-semibold text-gray-900">
                      {reserva.tipo_transaccion ? 
                        `${reserva.tipo_transaccion.charAt(0).toUpperCase() + reserva.tipo_transaccion.slice(1)} #${reserva.id}` :
                        `Reserva #${reserva.id}`
                      }
                    </h3>
                    ...
                    <Button
                      ...
                      className={favoritos.has(reserva.id) ? "text-yellow-500" : "text-gray-400"}
                    >
                      <Star className="w-4 h-4" fill={favoritos.has(reserva.id) ? "currentColor" : "none"} />
                    </Button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                    <div className="flex items-center">
                      ...
                      <span>{reserva.paquete?.nombre || 'Paquete no disponible'}</span>
                    </div>
                    <div className="flex items-center">
                  ...
                  {reserva.metodo_pago && (
                    <div className="mt-2 text-sm text-gray-600">
                      <span className="font-medium">💳 Método de pago:</span> {reserva.metodo_pago}
                    </div>
                  )}
                  
                  {reserva.motivo_cancelacion && (
                    <div className="mt-2 text-sm text-red-600">
                      <span className="font-medium">❌ Motivo:</span> {reserva.motivo_cancelacion}
                    </div>
                  )}
                  
                  <div className="mt-2 flex items-center justify-between">
                    <span className="text-lg font-semibold text-gray-900">
                      {reserva.tipo_transaccion === 'reprogramacion' ? 'Sin costo' : `${formatearMoneda(reserva.total)} ${reserva.moneda}`}
                    </span>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <Badge className={`ml-2 ${obtenerColorEstado(selectedReserva.estado)}`}>
                        {getEstadoTexto(selectedReserva.estado)}
                      </Badge>
                    </div>
                    <div>
                      <span className="text-gray-600">Paquete:</span>
                      <span className="ml-2 font-medium">{selectedReserva.paquete?.nombre || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Fecha de inicio:</span>
                      <span className="ml-2">{formatFecha(selectedReserva.fecha)}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Fecha de fin:</span>
                      <span className="ml-2">{selectedReserva.fecha_fin ? formatFecha(selectedReserva.fecha_fin) : 'No especificada'}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Número de personas:</span>
                      <span className="ml-2">{selectedReserva.numero_personas || 1}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Precio total:</span>
                      <span className="ml-2 font-semibold">{formatearMoneda(selectedReserva.total)} {selectedReserva.moneda}</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-sm space-y-1">
                    <div>
                      ...
                      <span className="ml-2">{formatFecha(selectedReserva.created_at)}</span>
                    </div>
                  </div>
                </div>
              </div>
